#!/usr/bin/env Rscript
#
# SuSiE fine-mapping from summary statistics and a precomputed LD matrix.
#
# Derived from FINNGEN/finemapping-pipeline (R/run_susieR.R), adding bcor-format
# LD input via mkanai/rbcor and susie_rss in place of susie_suff_stat.
#
...
load_R <- function(path, snp, dominant = FALSE, triangular_ld_matrix = FALSE) {
  ...
  } else if (tools::file_ext(path) == "bcor") {
    if (!require(rbcor)) {
      stop("rbcor package is required to read bcor files")
    }
    bcor <- rbcor::read_bcor(path)
    R <- bcor$read_corr()
  } else {
