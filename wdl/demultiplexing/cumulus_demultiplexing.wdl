...
import "https://raw.githubusercontent.com/mkanai/cumulus/fff16f1ab3ca0e7e7660f090fde60ead83c7024b/workflows/demultiplexing/demuxEM.wdl" as dem
import "https://raw.githubusercontent.com/mkanai/cumulus/fff16f1ab3ca0e7e7660f090fde60ead83c7024b/workflows/demultiplexing/souporcell.wdl" as soc
import "https://raw.githubusercontent.com/mkanai/cumulus/fff16f1ab3ca0e7e7660f090fde60ead83c7024b/workflows/demultiplexing/popscle.wdl" as psc
...
workflow demultiplexing {
    input {
        ...
        # Customized docker registry
        String popscle_docker_registry = docker_registry
    ...
    if (length(Config.pooling_ids) > 0) {
            ...
            if (demultiplexing_algorithm == "popscle") {
                call psc.popscle as popscle {
                    input:
                        ...
                        docker_registry = popscle_docker_registry,
        ...
        if (demultiplexing_algorithm == "popscle") {
            call merge_best {
                input:
                    best_files = select_all(popscle.output_best),
                    output_dir = output_directory_stripped,
                    popscle_version = popscle_version,
                    docker_registry = popscle_docker_registry,
                    zones = zones,
...
task merge_best {
    ...
    command <<<
        ...
        FNR == 1 {
            ...
            split(a[n], b, "-")
            OUTNAME = b[1]
            if (OUTNAME in seen == 0) {
                seen[OUTNAME] = 1
                print $0 > OUTNAME".best"
            }
        }
        FNR > 1 {
            count[OUTNAME"-"$col["DROPLET.TYPE"]] = count[OUTNAME"-"$col["DROPLET.TYPE"]] + 1
            sub(/-1$/, "", $col["BARCODE"])
            $col["BARCODE"] = OUTNAME"-"$col["BARCODE"]
            print $0 > OUTNAME".best"
        }
        END {
            for (i in seen) {
                print i, count[i"-SNG"], count[i"-DBL"], count[i"-AMB"] > i".count.txt"
            }
        }' ~{sep=" " best_files}

        strato cp --backend ~{backend} -m *.best *.count.txt "~{output_dir}"
    >>>

    output {
        Array[String] out_best = glob("*.best")
    }
