...
workflow munge_mesc {
    ...
    scatter (cell_type in cell_types) {
        ...
        String prefix = "~{basename(sumstats, '.cis_qtl_pairs.txt.bgz')}.mesc"

        scatter (chrom in chromosomes) {
            String prefix_chrom = "~{prefix}.~{sub(chrom, '^chr', '')}"
            call munge_saige_qtl {
                input:
                    chrom = chrom,
                    sumstats = sumstats,
                    bed = bed,
            ...
            call tasks.compute_expscore {
                input:
                    sumstats = munge_saige_qtl.out_sumstats,
                    prefix = prefix,
                    prefix_chrom = prefix_chrom,
                    docker = docker_ldsc,
                    zones = zones
...
task munge_saige_qtl {
    ...
    command <<<
        ...
        FNR > 1 && filenum == 3 && $col["SE"] != "NA" {
            if ($col["MarkerID"] in rsid) {
                snp = rsid[$col["MarkerID"]]
            } else {
                snp = $col["MarkerID"]
            }
            chrom = $col["#CHR"]
            ...
            z = $col["BETA"] / $col["SE"]
            print gene, gene_mid_pos, snp, chrom, $col["POS"], n, z | "sort -k2,2n -k5,5n"
        }
